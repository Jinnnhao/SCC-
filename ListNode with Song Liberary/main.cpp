...
void addThisSong(LinkedList<Song> &list, Song s){
 //TODO
    list.insert(s, s.getChartPosition()); // insert the song to its chart position
    
    int found = list.get(s);  // will return the song index position number if the song exist in the database,
  if (found >=0)
  {
      std::cout<<"ADDED THIS SONG "<<found+1<<" " ;
...
void processQueue(LinkedList<Song> &list ,Queue<Request>& q){
   ...
   //Create an iterator for the Queue to iteratate through requests
    Queue<Request>:: Iterator iter;
      
    // out put all the request except the last one
    for(iter = q.begin(); !(iter == q.end()); ++iter){
       std::cout<<"-------------------------------------------------------------------------\n";
         // call processRequest
        std::cout << "Request Number: " <<  requestNumber << " " << iter.getCur()->getData().toString()<< std::endl;
        
        processRequest(list, iter.getCur()->getData().getAction(), iter.getCur()->getData().getTitle(), iter.getCur()->getData().getSinger(), iter.getCur()->getData().getChartPosition());
        
        requestNumber++;
    std::cout<<"-------------------------------------------------------------------------\n";
    }
    
    // out put the last request
    std::cout << "Request Number: " <<  requestNumber << " " << iter.getCur()->getData().toString()<< std::endl;
    processRequest(list, iter.getCur()->getData().getAction(), iter.getCur()->getData().getTitle(), iter.getCur()->getData().getSinger(), iter.getCur()->getData().getChartPosition());
