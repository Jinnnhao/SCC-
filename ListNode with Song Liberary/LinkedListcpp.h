...
template <class T>
void LinkedList<T>::add( T  t){ // add node at the end so that first in first out
...
void LinkedList<T>::move(T t, int pos){
  //TODO
    ...
    // if the song is found -> cur == t
    if(cur->data == t)
    {
        prev->next = cur->next; // remove the current song frist and adjust the rest chartposition number -1
        adjustInsert(cur->next, -1);
        
        // then insert t to the position and also adjust the chartposition number in the insert function
        insert(t, pos);
    }
    
    // if the song is not found -> cur == nullptr
    else{
